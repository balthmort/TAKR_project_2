	...
	"""Sčítač četnosti písmen"""
     ...
     def getFrekvenciPoradek(zprava):
		"""Uspořádání písmen dle četnosti"""
         	pismenoToFreq = getPismenoScitat(zprava)
        	...
        	for pismeno in PISMENA:
         	...
         	for freq in freqTopismeno:
             		freqTopismeno[freq].sort(key=ETAOIN.find, zmena=True)
             		freqTopismeno[freq] = ''.join(freqTopismeno[freq])


         	freqPairs = list(freqTopismeno.items())
         	freqPairs.sort(key=getItemAtIndexZero, zmena=True)
		"""seznam n-tic (páru), které jsme setřídili"""
         	freqOrder = []
     	...
     	def frekvenceShody(zprava):
         	...
         	return shoda
		"""return shoda by měla vracet klíč,"""		
